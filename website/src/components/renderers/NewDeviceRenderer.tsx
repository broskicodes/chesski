...
export const NewDeviceRenderer = () => {
  ...
  useEffect(() => {
    if (!loading) renderDisconnected();
  }, [renderDisconnected, loading]);

  useEffect(() => {
    if (username) {
      newAlert(
        `You are currently connected as user "${username}". Proceeding will result in loss of access to data for that user.`,
        "default",
      );
    }
  }, [username, newAlert]);

...
import { GalleryContext, GalleryProviderContext, GameType, PgnFile, TimeControlMap, TimeControls } from "./context";
...
export const GalleryProvider = ({ children }: PropsWithChildren) => {
  ...
  const getFilesFromWnfs = useCallback(async () => {
    ...
    await Promise.all(Object.values(GameType).map(async(type) => {
    ...
    const files = (
      await Promise.all(
        Object.entries(links).map(async ([dirname]) => {
  ...
  const parsePgnToFiles = useCallback(async (pgnData: Uint8Array) => {
    if (!isConnected() || !fs) {
      return;
    }
    ...
    await Promise.all(pgns.map(async (pgn) => {
      const timeControl = pgn.headers?.at(9)?.value as string;
      const dirname = TimeControlMap[timeControl as TimeControls] ?? GameType.Other;
      ...
      const name = Buffer.from(await window.crypto.subtle.digest("SHA-256", Buffer.from(identifier))).toString("hex");
